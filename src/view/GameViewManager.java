...
public class GameViewManager {
    ...
    private final static String METEOR_GREY_IMAGE = "/view/resources/meteorGrey_big2.png";
    ...
    private void checkIfElementsCollide(){
        if(SHIP_RADIUS + STAR_RADIUS > calculateDistance(ship.getLayoutX() + 49, star.getLayoutX() + 15, ship.getLayoutY() + 37, star.getLayoutY() + 15)){
            setNewElementPosition(star);
            points++;
            String nextToText = "POINTS: ";
            if(points < 10){
                nextToText = nextToText + "0";
            }
            pointsLabel.setText(nextToText);
        }
        for(int i = 0; i < brownMeteors.length; i++){
